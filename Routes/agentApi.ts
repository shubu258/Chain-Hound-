...
import { attachEnsNames } from './traceFetching.js';
import { noopProgress } from '../src/progress.js';
...
router.post('/agent/trace', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const nodes = await attachEnsNames(result.rootWallet, result.nodes, noopProgress);
