#!/usr/bin/env bash
# Smoke-tests POST /api/trace.
#
# Usage:
#   ./scripts/test-trace-endpoint.sh
#
...
if [[ "$status" == "200" ]]; then
  if grep -q '"nodes":\[[^]]' "$RESP_FILE" && grep -q '"edges":\[' "$RESP_FILE"; then
    echo "PASS  well-formed walletAddress (status $status, nodes non-empty)"
    pass=$((pass + 1))
  else
    echo "FAIL  well-formed walletAddress (status $status, but nodes array was empty)"
    fail=$((fail + 1))
  fi
elif [[ "$status" == "502" ]]; then
  echo "PASS  well-formed walletAddress (status $status — upstream call failed, e.g. missing API keys)"
  pass=$((pass + 1))
else
  echo "FAIL  well-formed walletAddress (expected 200 or 502, got $status)"
  fail=$((fail + 1))
fi
echo "      $(cat "$RESP_FILE")"
echo
